class EasyInstantMessagesController < ApplicationController
  ...
  def still_online
    if User.find(params[:user_id]).update(last_seen_at: DateTime.now)
      render nothing: true, status: 200
    else
  ...
  #This method is added in order to override a setting that makes this plugin not work properly
