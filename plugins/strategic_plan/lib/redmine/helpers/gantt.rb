    ...
    class Gantt
      ...
      def render(options={})
        options = {:top => 0, :top_increment => 20,
                   :indent_increment => 10, :render => :subject,
      ...
      def render_issues(issues, options={})
        ...
        issues.each do |issue|
          ...
          render_object_row(issue, options) unless (issue.status.name == "En gestión permanente")
          unless issue.leaf?
            ancestors << issue
      ...
      def to_pdf
        ...
        subject_width = PDF::LeftPaneWidth - 20
        header_height = 5
      ...
      def html_subject(params, subject, object)
        style = "position: absolute;top:#{params[:top]}px;left:0px;"
      ...
      def pdf_subject(params, subject, options={})
        ...
        params[:pdf].SetX(15)
        char_limit = PDF::MaxCharactorsForSubject - params[:indent] - 20
      ...
      def pdf_task(params, coords, markers, label, object)
        ...
        # Renders the label on the right
        if label && object.is_a?(Issue)
          params[:pdf].SetX(params[:subject_width] + (coords[:bar_end] || 0) + 2)
