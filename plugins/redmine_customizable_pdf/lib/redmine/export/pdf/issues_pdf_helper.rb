      ...
      module IssuesPdfHelper
        ...
        def issue_to_pdf(issue, assoc={})
          ...
          if assoc[:journals].present?
            #if Setting.plugin_redmine_customizable_pdf.include?('disabled_details')
            pdf.SetFontStyle('B',9)
            ...
            assoc[:journals].each do |journal|
              if journal.notes?
                pdf.SetFontStyle('B',8)
                title = "#{format_time(journal.created_on)} - #{journal.user}"
          ...
          # Dejo comentado el codigo que dibuja los attachment en los PDFs. Por el momento no los quieren.
