( function( window ) {
  ...
  $(window).load(function() {
    ...
    $( 'input[name$="q"]' ).attr( 'placeholder','Buscar...' );
    if (activeStaticSidebar) {
  ...
  function readaptLogin() {
    var samllogin = $('.action-login #saml-login').remove();
    var normallogin = $('.action-login #login-form').remove();

    $('.action-login #content').prepend('<div id="login"></div>');
    $('#login').prepend(normallogin);
    $('#login').prepend(samllogin);
  }
  $(document).ready(readaptLogin);

  /*function addCollapseMenu() {
    $( '#projects-index ul.projects li.root>ul.projects' ).accordion({
      collapsible: true,
      active: false
    });

    $( '#projects-index ul.projects li.child>ul.projects' ).accordion({
      collapsible: true,
      active: false
    });
